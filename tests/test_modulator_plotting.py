...
def test_plot_bias_spectra_can_use_frequency_axis(tmp_path: Path):
    rows = [
        {
            "wavelength": 1.54,
            "voltage": 0.0,
    ...
    output_path = plot_bias_spectra(
        ...
        y_axis="db",
        x_axis="frequency",
        output_path=tmp_path / "frequency.png",
    )

    assert output_path.exists()


def test_plot_bias_spectra_rejects_unknown_x_axis(tmp_path: Path):
    with pytest.raises(ValueError):
        plot_bias_spectra([], x_axis="time", output_path=tmp_path / "bias.png")
